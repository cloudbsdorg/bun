...
const actualNodeErrors: any[] = (NodeErrors as any).default || NodeErrors;
const extra_count = actualNodeErrors
  .map(x => x.slice(3))
  .filter(x => x.length > 0)
  .reduce((ac, cv) => ac + cv.length, 0);
const count = actualNodeErrors.length + extra_count;
...
let i = 0;
for (let [code, constructor, name, ...other_constructors] of actualNodeErrors) {
...
let builtindtsPath = path.join(__dirname, "..", "..", "src", "js", "builtins.d.ts");
let builtindts = await (globalThis.Bun ? Bun.file(builtindtsPath).text() : fs.promises.readFile(builtindtsPath, "utf-8"));
...
`;
for (const [code, constructor, name, ...other_constructors] of actualNodeErrors) {
