...
<Warning>
  ...
  Some cron expressions that work on Linux, macOS, and FreeBSD exceed this limit on Windows. When a pattern exceeds the limit,
...
**Expressions that fail on Windows** (but work on Linux, macOS, and FreeBSD):
...
| -------- | -------------------------------------------------------- |
| Linux/FreeBSD | Edits crontab to remove the entry and its marker comment |
