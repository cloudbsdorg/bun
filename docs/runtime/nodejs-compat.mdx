...
🟡 Handles and file descriptors cannot be passed between workers, which means load-balancing HTTP requests across processes is only supported on Linux and FreeBSD at this time (via `SO_REUSEPORT`). Otherwise, implemented but not battle-tested.
